...
class CirclesApplicationSession: ObservableObject {
    ...
    public func roomIsKnown(roomId: RoomId) -> Bool {
        ...
        if let _ = self.matrix.invitations[roomId] {
            return true
        }
    ...
    public func onOpenURL(url: URL) {
        ...
        guard let _ = self.matrix.rooms[roomId]
        else {
        ...
        case "group":
            ...
            if let _ = self.groups.rooms[roomId] {
        ...
        case "gallery":
            ...
            if enableGalleries {
                if let _ = self.galleries.rooms[roomId] {
    ...
    public func navigate(to roomId: RoomId) async throws {
        ...
        if let room = try? await self.matrix.getRoom(roomId: roomId) {
            ...
            case "m.space":
                if let _ = self.people.rooms[roomId] {
